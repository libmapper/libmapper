----------------------------------------------------------------------
-- Premake4 configuration script for libmapper
...
--  io.output("../src/lo_endian.h")
--  local text = io.read("*all")
--  io.write(text)
--  io.close()

----------------------------------------------------------------------
-- The libmapper library project
----------------------------------------------------------------------
  ...
  project "libmapper"
    ...
    configuration { "*Lib" }
      kind    "StaticLib"
      defines "LIBMAPPER_LIB"
      
    configuration { "*DLL" }
      kind    "SharedLib"
      defines "LIBMAPPER_DLL"

    configuration { "Debug*" }
      targetname "libmapper_d"
      
    configuration { "Release*" }
      targetname "libmapper"
