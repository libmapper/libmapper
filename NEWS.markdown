...
leave `y[1]` unchanged.
* *Vector functions.* Several functions the address entire vectors have been added:
...
source signals are connected to the same destination signal. Prior to 0.4,
...
overwrite each other at each source update. Starting with version 0.4, three
...
reducing network traffic. Databases are now properly notified of signal removal.
...
in libmapper and to improve consistency. These bindings now support the full
functionality of libmapper including database queries.
