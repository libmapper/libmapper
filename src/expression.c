   ...
   while (i_s<=strlen(s))
    {
        ...
        if ( is_separator(s[i_s])!=0 && ( (i_s==0) || ((i_s!=0)&&(s[i_s-1]!='[')) ) )
        /* '-' must not be seen as a separator when it is used for history values : x[-1]...*/
            {

                /* If the previous separator is not the previous character, a new word has to be inserted into the array*/
                if (i_prev_separator != (i_s-1))
                    {
                        tab[i_tab]=(char*) malloc((i_s-i_prev_separator)*sizeof(char));
                        tab[i_tab]=sub_string(s, i_prev_separator+1, i_s-1);
                        i_tab++;
                    }
                /* If this separator is different from '\0', it has to be inserted into the array*/
...
while(i<t2+1)
    {
        ...
        else if ( isdigit(expr[i][0])!=0 || ( strcmp(expr[i],"pi") == 0) || ( strcmp(expr[i],"Pi") == 0) )
            {
                        ...
                        switch(expr[i][0])
                            {
                                ...
                                /* '-' represents both operators MINUS (arity_2) and OPP (arity_1)*/
                                case '-':
                                if ( (i==t1) || ( (i!=t1)&&(expr[i-1][0]=='(') ) || ( (i!=t1)&&(expr[i-1][0]=='*') ) || ( (i!=t1)&&(expr[i-1][0]=='/') )  || ( (i!=t1)&&(expr[i-1][0]=='^') ))
                                tmp_oper=string_to_operator("-",arity_1, err);
                                else tmp_oper=string_to_operator("-", arity_2, err);
                                break;
...
if (curr_prior==PRIOR_VAR)
    {
        /*In this case T->num gives the history order, and not a nuveric value*/
        T->oper=operat;
        if (history_order!=0) T->num=history_order;
    }
...
else if (curr_prior==PRIOR_NUMBER)
    {
        ...
        else if (( strcmp(expr[positionv],"pi") == 0) || ( strcmp(expr[positionv],"Pi") == 0))
            {
                T->num=_Pi;
            }
        else
...
else if (operat.type==arity_1)
    {
        ...
        if ( (t2-prev_parenth_order-position-1)<0 || (expr[position+1][0]=='('&&expr[position+2][0]==')'))
            {
                *err=ERR_MISSING_ARG;
        ...
        ConstructTree(T->left, expr, position+1,t2-prev_parenth_order, err);
    }
...
else if (operat.id==OPP.id)
    {
        ...
        ConstructTree(T->right, expr, position+1,t2-prev_parenth_order, err);
    }
...
else
    {
        ...
        ConstructTree(T->left, expr, t1+prev_parenth_order,position-1, err);
