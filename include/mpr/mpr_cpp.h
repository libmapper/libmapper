...
namespace mpr {
    ...
    /*! libmpr uses NTP timetags for communication and synchronization. */
    class Time
    {
    public:
        Time(mpr_time time)
...
//            { return Device::List((void**)mpr_map_scopes(_obj)); }
     ...
     *  uses libmpr.  A Device must have a name, to which a unique ordinal is
    ...
    {
    public:
        /*! Create a peer in the libmpr distributed graph.
...
std::ostream& operator<<(std::ostream& os, const mpr::Property& p)
{
...
std::ostream& operator<<(std::ostream& os, const mpr::Object& o)
{
    mpr_obj obj = (mpr_obj)o;
    switch (mpr_obj_get_type(obj)) {
        case MPR_DEV: os << mpr::Device(obj); break;
        case MPR_SIG: os << mpr::Signal(obj); break;
        case MPR_MAP: os << mpr::Map(obj);    break;
        default:                              break;
    }
    return os;
}
