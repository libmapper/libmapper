...
These tutorials introduce new users to _libmpr_, providing steps to construct software programs that are compatible with the _libmpr_ network.  A program typically implements a libmpr **device**, and declares **signals**, which can be inputs or outputs. After this, run-time **maps** can be easily created and configured between declared signals. libmpr takes care of announcing and discovering available resources on the network, creating and maintaining connections between entities, and various other functions.

<img style="display:block;margin:auto;padding:0px;width:75%" src="./images/network_diagram.png">

The diagram above shows a simplified network of libmpr-enabled devices. In real use, devices and signals are expected to be given short descriptive names rather than the identifiers used in this example.

* A **signal** is a producer or consumer of real-time data. Signals in libmpr
...
1. **start a libmpr "device"**
2. **add some signals**
3. **update any outputs periodically**
4. **poll the device to process incoming messages**

Detailed tutorials are provided for using libmpr in the following programming languages and environments:
