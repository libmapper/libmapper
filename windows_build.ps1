...
if (!(Test-Path "$($scriptDir)\build\liblo\")) {
  Invoke-WebRequest https://github.com/radarsat1/liblo/archive/refs/tags/0.36.zip -OutFile liblo.zip
  Expand-Archive liblo.zip liblo
  rm liblo.zip
  cd liblo\liblo-0.36\cmake\
...
cp -v ./build/liblo/liblo-0.36/cmake/build/Release/liblo.dll ./dist/liblo.dll
